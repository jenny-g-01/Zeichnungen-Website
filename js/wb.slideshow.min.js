/* jQuery Slideshow Plugin for WYSIWYG Web Builder 16 - Copyright Pablo Software solutions 2021 - http://www.wysiwygwebbuilder.com */
...
String(g+1)).css("cursor","pointer");a("<img />").attr({id:"page_"+String(g+1),src:c.pagination_img_default}).appendTo(f);f.click(function(){a.slideshow.gotoImage(b,a(this).attr("id"))});f.appendTo($pagination)}$pagination.appendTo(a(b));a.slideshow.updateNavigation(b,0,c.last)}setTimeout(function(){a(b).trigger("activate",0)},10)};a.slideshow.killTimer=function(b){b=a(b).data("timerId");clearInterval(b)};a.slideshow.updateNavigation=function(b,c,d){var e=a(b).data("state");a(b).parent().find(".pagination img:eq("+
c+")").attr("src",e.pagination_img_default);a(b).parent().find(".pagination img:eq("+d+")").attr("src",e.pagination_img_active)};a.slideshow.transformRotate=function(b,c,d){a(b).css({"-webkit-transform":"rotate"+c+"("+d+"deg)","-moz-transform":"rotate"+c+"("+d+"deg)","-ms-transform":"rotate"+c+"("+d+"deg)","-o-transform":"rotate"+c+"("+d+"deg)",transform:"rotate"+c+"("+d+"deg)"})};a.slideshow.gotoImage=function(b,c){a.slideshow.killTimer(b);var d=a(b).data("state");0!=d.interval&&(d.current--,0>d.current&&
(d.current=d.images.length-1));c-1!=d.current&&(0==d.interval&&(d.last=d.current),d.current=Math.min(c-1,d.images.length-1),d.interval=0,d.pagination&&a.slideshow.updateNavigation(b,d.last,d.current),a.slideshow.imagetransition(b),d.last=d.current)};a.slideshow.nextimage=function(b){state=a(b).data("state");0!=state.interval&&(state.current=state.last);state.interval=0;a.slideshow.next(state);a.slideshow.imagetransition(b)};a.slideshow.previmage=function(b){state=a(b).data("state");0!=state.interval&&
(state.current=state.last);state.interval=0;0<=state.current-1?(--state.current,state.last=state.current+1):(state.current=state.images.length-1,state.last=0);a.slideshow.imagetransition(b)};a.slideshow.next=function(b){if("sequence"==b.type)b.current+1<b.images.length?(b.current+=1,b.last=b.current-1):(b.current=0,b.last=b.images.length-1);else if("random"==b.type)for(b.last=b.current;b.current==b.last;)b.current=Math.floor(Math.random()*b.images.length)};a.slideshow.imagetransition=function(b){state=
a(b).data("state");if(state.random){var c="fade slide puff blind drop zoom clip shuffle blast blocks rain swirl squares bars".split(" "),d=["left","up","down","right"],e=["horizontal","vertical"];state.effect=c[Math.floor(Math.random()*c.length)];if("slide"==state.effect||"drop"==state.effect)state.direction=d[Math.floor(Math.random()*d.length)];if("clip"==state.effect||"blinds"==state.effect||"bars"==state.effect)state.direction=e[Math.floor(Math.random()*e.length)]}for(c=0;c<state.images.length;c++)a(state.images[c]).css("display",
...
height:Math.ceil(heightStart)}).animate({opacity:1},2E3).animate({left:Math.ceil(leftEnd),top:Math.ceil(topEnd),width:Math.ceil(widthEnd),height:Math.ceil(heightEnd)},{duration:state.effectlength+2E3,queue:!1}).css({width:d,height:c});a(b).trigger("activate",state.active)}else if("zoom"==state.effect)e=a(state.images[state.last]).outerWidth(),g=a(state.images[state.last]).outerHeight(),d=a(b).outerWidth(),c=a(b).outerHeight(),d=(d-e)/2,f=(c-g)/2,0!=state.fullscreen&&(f=0),c={top:f,left:d,width:e,
height:g},d={width:0,height:0,top:f+g/2,left:d+e/2},a(state.images[state.current]).animate(d,0,null),a(state.images[state.current]).css("display","block").css("zIndex","1"),a(state.images[state.current]).animate(c,state.effectlength,function(){a(b).trigger("activate",state.active)});else if("zoominout"==state.effect)e=a(state.images[state.last]).outerWidth(),g=a(state.images[state.last]).outerHeight(),d=a(b).outerWidth(),c=a(b).outerHeight(),d=(d-e)/2,f=(c-g)/2,0!=state.fullscreen&&(f=0),c={top:f,
